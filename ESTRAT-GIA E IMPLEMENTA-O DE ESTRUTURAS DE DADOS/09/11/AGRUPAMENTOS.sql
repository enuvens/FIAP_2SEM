...
SELECT CD_CD FROM T_SCC_CD;
SELECT MAX(CD_CD)+1 FROM T_SCC_CD;
...
-- EXEMPLO UTILIZANDO HAVING
  SELECT F.CD_DEPTO , D.NM_DEPTO, AVG(F.VL_SALARIO) "MÉDIA SALÁRIOS"
    FROM T_SIP_FUNCIONARIO F INNER JOIN T_SIP_DEPARTAMENTO D ON (D.CD_DEPTO = F.CD_DEPTO)
   WHERE F.CD_DEPTO  <> 3 -- FILTRA TABELA
GROUP BY F.CD_DEPTO, D.NM_DEPTO
  HAVING AVG(F.VL_SALARIO) > 2000 -- FILTRA GRUPOS
ORDER BY AVG(F.VL_SALARIO) DESC;
